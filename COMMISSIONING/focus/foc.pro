...
domainnm = host[1]
  ...
  imo = double(readfits(filename, header, /silent))
if keyword_set(plt) then display,imo,/log, /silent

pos = double(sxpar(header, 'focus'))
if keyword_set(pr) then print, 'The focus position for this image was: ', pos, 'mm.'
...
if domainnm eq 'ctio.noao.edu' then begin
  readcol, '/mir7/pro/chiron_lines.found', x, y, format='a,a'
endif else begin
  readcol, 'chiron_lines.found', x, y, format='a,a', /silent
endelse
...
;MARK the LOCATIONS of the FOCUS Th LINES
 ...
 ;    ys=indgen(600)
     ...
     xt='COL #'
     !p.color=300
window, 0, xsize=2000, ysize=1400
     display,im,xs,ys,titl=titl,ytit=yt,xtit=xt,/log, min=250, max=50000d, /silent;,/log
...
;window, 0, xsize=3700, ysize=1300, xpos=200
     ;display,im,xs,ys,/log; min=20, max=50000d;,/log
        ...
        FOR j=0,nl-1 do begin
           ...
           if keyword_set(pr) then print,colloc,rowloc, im[x[j], y[j]]
     ...
     ENDIF ELSE BEGIN        ;END FIND LINE LOCATIONS
	  ...
	  if keyword_set(pr) then print, 'YOU ENTERED MARK, BUT NO FINDLOC'
     endelse
...
; Use Gaussian to get height and width
    ...
    if keyword_set(pr) then print, 'now on line: ', j
;    print, fit
